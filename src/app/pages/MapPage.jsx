...
export default function MapPage() {
  ...
  const { ngos, donations, geocodeHealth } = useAppState();
  ...
  const mapCenter = selectedDonation
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <Card className={`mb-6 ${geocodeHealth.fallbackRate > 0 ? 'border-amber-300 bg-amber-50' : 'border-green-200 bg-green-50'}`}>
          <CardContent className="p-4 text-sm text-gray-700">
            <div className="font-medium mb-1">Geocoding Health</div>
            <div>
              Total requests: {geocodeHealth.totalRequests} | Cache: {geocodeHealth.cacheCount} | Live provider: {geocodeHealth.nominatimCount} | Fallback: {geocodeHealth.fallbackCount} ({geocodeHealth.fallbackRate}%)
            </div>
            {geocodeHealth.fallbackRate > 0 && (
              <div className="mt-1 text-amber-800">
                Some addresses used fallback coordinates. Ask donors for clearer landmark-rich addresses for better routing precision.
              </div>
            )}
          </CardContent>
        </Card>
            ...
            <CardContent className="space-y-3">
              {Object.entries(radiusConfig).map(([category, config]) => (
                <button
                  ...
                  style={{
                    backgroundColor:
                      selectedCategory === category ? `${config.color}14` : 'white',
                  }}
                >
              ...
              <div className="h-[620px]">
                ...
                >
                  ...
                  {selectedDonation && (
                    <>
                      <Marker
                        position={[selectedDonation.pickupLat, selectedDonation.pickupLng]}
                        icon={donorIcon}
                      >
                        <Popup>
                          <div className="space-y-1">
                            ...
                            <div>Geocode source: {selectedDonation.geocodeSource || 'unknown'}</div>
                          </div>
                        </Popup>
                      </Marker>
                      ...
                      <Circle
                        ...
                        pathOptions={{
                          color: currentRadius.color,
                          fillColor: currentRadius.color,
                          fillOpacity: currentRadius.fillOpacity,
                        }}
                      />
                    </>
                  )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
            <Card>
              ...
              <CardContent className="space-y-2 text-sm text-gray-700">
                ...
                <div><span className="font-medium">Distance Estimate:</span> {selectedDonation.distance} km</div>
                <div><span className="font-medium">ETA:</span> {selectedDonation.etaMinutes || 'N/A'} mins ({selectedDonation.routeMode || 'straight-line'})</div>
              </CardContent>
            </Card>
